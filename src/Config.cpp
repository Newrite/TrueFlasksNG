...
namespace config
{
  ...
  private:
    ...
    [[nodiscard]] auto parse_keyword(const std::string& val) -> RE::BGSKeyword*
    {
      ...
      if (!res.has_value())
        return nullptr;
      auto form = core::utility::get_form_from_form_id_and_mod_name(
        res->id, res->mod_name);
      if (form) {
    ...
    [[nodiscard]] auto parse_sound_descriptor(const std::string& val)
      -> RE::BGSSoundDescriptorForm*
    {
      ...
      if (form) {
        if (auto kw = form->As<RE::BGSSoundDescriptorForm>()) {
          logger::info("Resolved sound descriptor: {} -> {:08X}", val,
                       kw->GetFormID());
          return kw;
        }
        else {
    ...
    [[nodiscard]] auto keyword_to_string(RE::BGSKeyword* keyword,
    ...
    [[nodiscard]] auto
    sound_descriptor_to_string(RE::BGSSoundDescriptorForm* sound_desc,
                               const std::string& default_val) -> std::string
    {
      if (!sound_desc)
        return default_val;
      return core::utility::get_form_id_hex_and_mod_name_as_string(sound_desc,
                                                                   true);
    }

    void read_flask_base(const mINI::INIStructure& ini,
                         ...
                         flask_settings_base& settings)
    {
      ...
      if (ini.has(section)) {
        ...
        if (collection.has("FlasksEnable"))
          parse_bool(collection.get("FlasksEnable"), settings.enable);
        if (collection.has("FlasksNPC"))
        ...
        if (collection.has("FlasksAntiSpam"))
          parse_bool(collection.get("FlasksAntiSpam"), settings.anti_spam);
        ...
        if (collection.has("FlasksRegenerationMultKeyword"))
          regen_kw = collection.get("FlasksRegenerationMultKeyword");
        if (collection.has("FlasksCapBase"))
          parse_int(collection.get("FlasksCapBase"), settings.cap_base);
        if (collection.has("FlasksCapKeyword"))
          cap_kw = collection.get("FlasksCapKeyword");
        if (collection.has("FlasksCooldownBase"))
          parse_float(collection.get("FlasksCooldownBase"),
                      settings.cooldown_base);
        if (collection.has("FlasksCooldownKeyword"))
          cd_kw = collection.get("FlasksCooldownKeyword");

        if (collection.has("FlasksFailAudio"))
          parse_bool(collection.get("FlasksFailAudio"), settings.fail_audio);
        if (collection.has("FlasksFailAudioSound"))
          fail_sound = collection.get("FlasksFailAudioSound");
      }
      else {
    ...
    void populate_ini(mINI::INIStructure& ini)
    {
      ini["TrueFlasksNG"]["NoRemoveKeyword"] =
        keyword_to_string(main.no_remove_keyword, "0x800~Mod.esp");

      auto write_flask = [&](const std::string& section,
                             const flask_settings_base& s) {
        ...
        ini[section]["FlasksRegenerationMultKeyword"] =
          keyword_to_string(s.regeneration_mult_keyword, "0x800~Mod.esp");
        ini[section]["FlasksCapBase"] = std::to_string(s.cap_base);
        ini[section]["FlasksCapKeyword"] =
          keyword_to_string(s.cap_keyword, "0x800~Mod.esp");
        ini[section]["FlasksCooldownBase"] =
          std::format("{:.1f}", s.cooldown_base);
        ini[section]["FlasksCooldownKeyword"] =
          keyword_to_string(s.cooldown_keyword, "0x800~Mod.esp");
        ini[section]["FlasksFailAudio"] = s.fail_audio ? "1" : "0";
        ini[section]["FlasksFailAudioSound"] =
          sound_descriptor_to_string(s.fail_audio_form, "0x800~Mod.esp");
      };

      write_flask("FlasksOther", flasks_other);
      ini["FlasksOther"]["FlasksOtherExclusiveKeyword"] =
        keyword_to_string(flasks_other.exclusive_keyword, "0x800~Mod.esp");
      ini["FlasksOther"]["FlasksRevertExclusive"] =
        flasks_other.revert_exclusive ? "1" : "0";
      ...
      auto write_prisma_flask = [&](const std::string& prefix,
        ...
        ini["PrismaWidget"]["PrismaFlasksFillAnimationOnlyZero" + type] =
          s.fill_animation_only_zero ? "1" : "0";
      };

      write_prisma_flask("PrismaFlasksHealth", "Health", prisma_widget.health);
      write_prisma_flask("PrismaFlasksStamina", "Stamina", prisma_widget.stamina);
      write_prisma_flask("PrismaFlasksMagick", "Magick", prisma_widget.magick);
      write_prisma_flask("PrismaFlasksOther", "Other", prisma_widget.other);
    }
  ...
  public:
    ...
    auto load() -> void
    {
      ...
      if (ini.has("TrueFlasksNG")) {
        ...
        if (sec.has("NoRemoveKeyword"))
          no_remove_kw = sec.get("NoRemoveKeyword");
      }
      ...
      if (ini.has("FlasksOther")) {
        ...
        if (sec.has("FlasksOtherExclusiveKeyword"))
          exclusive_kw = sec.get("FlasksOtherExclusiveKeyword");
      ...
      if (ini.has("PrismaWidget")) {
        ...
        if (sec.has("PrismaEnable"))
          parse_bool(sec.get("PrismaEnable"), prisma_widget.enable);
        if (sec.has("AutoHideUI"))
          parse_bool(sec.get("AutoHideUI"), prisma_widget.auto_hide_ui);
        if (sec.has("AlwaysShowInCombat"))
          parse_bool(sec.get("AlwaysShowInCombat"),
                     prisma_widget.always_show_in_combat);
        if (sec.has("PrismaPositionX"))
          parse_float(sec.get("PrismaPositionX"), prisma_widget.x);
        if (sec.has("PrismaPositionY"))
          parse_float(sec.get("PrismaPositionY"), prisma_widget.y);
        if (sec.has("PrismaSize"))
          parse_float(sec.get("PrismaSize"), prisma_widget.size);
        if (sec.has("PrismaOpacity"))
          parse_float(sec.get("PrismaOpacity"), prisma_widget.opacity);
        if (sec.has("PrismaAnchorAllElements"))
          parse_bool(sec.get("PrismaAnchorAllElements"),
                     prisma_widget.anchor_all_elements);

        auto read_prisma_flask = [&](const std::string& prefix,
                                     const std::string& type,
                                     prisma_flask_widget_settings& s) {
          if (sec.has(prefix + "X"))
            parse_float(sec.get(prefix + "X"), s.x);
          if (sec.has(prefix + "Y"))
            parse_float(sec.get(prefix + "Y"), s.y);
          if (sec.has(prefix + "Size"))
            parse_float(sec.get(prefix + "Size"), s.size);
          if (sec.has(prefix + "Opacity"))
            parse_float(sec.get(prefix + "Opacity"), s.opacity);

          if (sec.has("PrismaFlasksFillAnimation" + type))
            parse_bool(sec.get("PrismaFlasksFillAnimation" + type),
                       s.fill_animation);
          if (sec.has("PrismaFlasksFillAnimationOnlyZero" + type))
            parse_bool(sec.get("PrismaFlasksFillAnimationOnlyZero" + type),
                       s.fill_animation_only_zero);
        };

        read_prisma_flask("PrismaFlasksHealth", "Health", prisma_widget.health);
        read_prisma_flask("PrismaFlasksStamina", "Stamina",
                          prisma_widget.stamina);
        read_prisma_flask("PrismaFlasksMagick", "Magick", prisma_widget.magick);
        read_prisma_flask("PrismaFlasksOther", "Other", prisma_widget.other);
      }
