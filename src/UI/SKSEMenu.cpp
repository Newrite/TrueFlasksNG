...
void render_flask_settings_group(const char* label, config::flask_settings_base& settings) {
    if (ImGui::CollapsingHeader(label)) {
        ...
        if (ImGui::DragFloat("Regeneration Mult Base", &settings.regeneration_mult_base, 1.0f, 0.0f, 1000.0f, "%.1f")) changed = true;
...
void __stdcall render_flasks_settings()
{
    ...
    if (ImGui::CollapsingHeader("Other Flasks")) {
        ...
        if (ImGui::Checkbox("Enable##Other", &settings.enable)) changed = true;
        if (ImGui::Checkbox("Enable for Player##Other", &settings.player)) changed = true;
        if (ImGui::Checkbox("Enable for NPC##Other", &settings.npc)) changed = true;
        
        char buffer[256];
        strncpy_s(buffer, settings.notify.c_str(), sizeof(buffer) - 1);
        if (ImGui::InputText("Notification##Other", buffer, sizeof(buffer))) {
            settings.notify = buffer;
            changed = true;
        }
        
        if (ImGui::Checkbox("Parallel Cooldown##Other", &settings.enable_parallel_cooldown)) changed = true;
        if (ImGui::Checkbox("Anti-Spam##Other", &settings.anti_spam)) changed = true;
        if (settings.anti_spam) {
            if (ImGui::DragFloat("Anti-Spam Delay##Other", &settings.anti_spam_delay, 0.1f, 0.0f, 10.0f, "%.1f")) changed = true;
        }
        
        if (ImGui::DragFloat("Regeneration Mult Base##Other", &settings.regeneration_mult_base, 1.0f, 0.0f, 1000.0f, "%.1f")) changed = true;
        if (ImGui::InputInt("Cap Base##Other", &settings.cap_base)) changed = true;
        if (ImGui::DragFloat("Cooldown Base##Other", &settings.cooldown_base, 1.0f, 0.0f, 3600.0f, "%.1f")) changed = true;
